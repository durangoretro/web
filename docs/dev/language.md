...
program is recommended; although it's possible to _manually_ translate menmonics
...
and learning purposes, the more limited [miniMoDA](6502/minimoda.md) may be used as well.
...
!!! tip
	...
	[tools](../tools.md) we supply, both [in Durango itself](insitu.md) and
	[in the host computer](tools.md), this is less of an issue.
...
to newcomers and non-technical people (much like _Python_ does today), even if it's
far from being the best in _code structure_ and, especially, _execution speed_, as
this is an **interpreted language**, thus every sentence is translated into machine code
_on the fly_.

### [EhBASIC](lang/ehbasic.md)
...
especially _Commodore_); but [out port](../assets/bin/basic_vsp.dux) adds **support
for _Durango·X_ graphics, colour and sound** plus **LOAD and SAVE** capabilities,
both for the [Virtual Serial Port]() if under _Perdita_ emulation, or even
...
which is credited with **efficient code** plus all the advantages of a _compiled_ language.
...
There's even a _proof-of-concept_ of a [Python](https://en.wikipedia.org/wiki/Python_(programming_language))
